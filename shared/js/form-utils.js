class FormUtils {
    ...
    static validatePassword(value) {
        if (!value) {
            return { isValid: false, message: 'Password es requerida' };
        }
        if (value.length < 8) {
            return { isValid: false, message: 'Es necesario un password de 8 caracteres' };
        }
        if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(value)) {
            return { isValid: false, message: 'el password debe de tener mayusculas o un numero' };
        }
        return { isValid: true };
    }
